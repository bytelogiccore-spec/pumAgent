...
async fn main() {
    ...
    println!("   -> ⏱️ {}개의 교란/함정 데이터(키워드 중복 포함) 주입 완료! ({:?})", num_noises, start_write.elapsed());
    ...
    knowledge.write("rules", "core_persona", core_content).unwrap();
    ...
    for i in 0..5 {
        ...
        stress_tasks.push(tokio::spawn(async move {
            let knowledge = KnowledgeTool::new(db_clone);
            for j in 0..100 {
                let _ = knowledge.write("workflows", &format!("bg_task_{}_{}", i, j), "background memory");
            }
        }));
    }
    ...
    println!("   -> 1000번의 동시 다발적 Read 요청 중 성공적으로 무결성을 보존한 횟수: {}/1000", hits_val);
    if hits_val == 1000 {
        println!("✅ [CONCURRENCY TEST SUCCESS] 심각한 동시성 간섭에도 기억의 변조나 분열이 없습니다!");
    } else {
