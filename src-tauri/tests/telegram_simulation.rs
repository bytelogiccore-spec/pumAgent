...
async fn test_telegram_logic_simulation() {
    ...
    config.planner_prompt =
        Some("[PLANNER] You MUST use tools for news. ALWAYS use <think> tags.".to_string());
    config.use_multi_agent_workflow = true;
    config
        .save(&base_dir)
        .expect("Failed to save config for test");
    ...
    let search_tool = app_lib::tools::search::SearchTool::new(db.clone(), base_dir.clone());
    let brain_tool = app_lib::tools::brain::BrainTool::new(db.clone());
    let terminal_tool =
        app_lib::tools::terminal::TerminalTool::new(base_dir.clone(), Some(db.clone()));
    ...
    let pumai_tool = app_lib::tools::pumai_tool::PumaiTool::new(base_dir.clone());
    ...
    let session_history = vec![ChatMessage {
        role: "user".to_string(),
    ...
    match res {
        Ok((final_out, history)) => {
            ...
            let any_tool_calls = history
                .iter()
                .any(|m| m.role == "assistant" && m.content.contains("\"tool\":"));
            if any_tool_calls {
            ...
            for m in history {
                if m.role == "assistant"
                    && (m.content.contains("<think>") || m.content.contains("<thought>"))
                {
