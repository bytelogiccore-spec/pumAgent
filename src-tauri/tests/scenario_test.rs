...
async fn test_moltbook_feed_scenario() {
    ...
    let constants_content =
        std::fs::read_to_string(&constants_path).expect("Failed to read constants.ts");
    let start_tag = "export const DEFAULT_PLANNER = `";
    let start_idx = constants_content
        .find(start_tag)
        .expect("Could not find DEFAULT_PLANNER")
        + start_tag.len();
    let end_idx = constants_content[start_idx..]
        .find("`;")
        .expect("Could not find end of DEFAULT_PLANNER")
        + start_idx;
    ...
    let response = client
        .chat(
            &[
                ChatMessage {
                    role: "system".into(),
                    content: actual_prompt.clone(),
                    images_base64: None,
                },
                ChatMessage {
                    role: "user".into(),
                    content: "몰트북 최신피드 5개 조사해줘".into(),
                    images_base64: None,
                },
            ],
            0.7,
        ...
        .await
        .expect("LLM 서버와 통신 실패 (llama-server가 켜져 있는지 확인하세요)");
    ...
    let debug_history = vec![
        ...
        ChatMessage {
            role: "user".into(),
            content: artificial_error.into(),
            images_base64: None,
        },
    ];

    let debug_response = client
        .chat(&debug_history, 0.7)
        .await
        .expect("LLM 통신 실패");
    println!(
        "에러 발생 후 플래너의 분석/복구 반응:\n{}",
...
async fn debug_vault_check() {
    ...
    match entry {
        Ok(e) => match e.get_password() {
            ...
            Err(err) => println!(
                "ERROR: Could not retrieve password for 'moltbook_creds': {}",
                err
            ),
        },
